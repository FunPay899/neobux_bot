...
async def successful_payment(message: Message, db: Database):
    ...
    if payload.startswith("buy_product:"):
    ...
    if payload.startswith("topup_balance:"):
        _, amount = payload.split(":")
        amount = int(amount)

        await db.add_balance(message.from_user.id, amount)
        await db.add_balance_topup(
            user_id=message.from_user.id,
            amount=amount,
            telegram_payment_charge_id=message.successful_payment.telegram_payment_charge_id,
            provider_payment_charge_id=message.successful_payment.provider_payment_charge_id,
        ...
        user = await db.get_user(message.from_user.id)
        current_balance = user.get("balance", 0) if user else amount
        ...
        admin_text = (
            ...
            f"Пользователь: @{message.from_user.username or 'без username'} (ID: {message.from_user.id})\n"
            f"Сумма: {amount} ⭐️\n"
            f"Баланс после пополнения: {current_balance} ⭐️"
        )
        for admin_id in settings.admin_ids:
            try:
                await message.bot.send_message(admin_id, admin_text)
